...
def _read_signals_csv(csv_path=CSV_PATH):
...
def _read_trades_sqlite(start_dt, end_dt):
...
def _plot_score_hist(df_signals, out_png):
...
def _plot_equity_curve(df_trades, out_png):
    """
    ...
    plt.title("Kümülatif PnL (Kapalı İşlemler)")
    plt.xlabel("Zaman")
    plt.ylabel("USD")
    plt.tight_layout()
    plt.savefig(out_png, dpi=150)
...
def generate_daily_report(csv_path=CSV_PATH):
    """
    Dünkü 00:00–23:59 veya bugün için günlük özet üretir.
    Basitlik adına 'bugün' raporlanır.
    """
    ...
    if df_signals is not None:
        df_signals_today = df_signals[
            (df_signals["ts"] >= start_dt) & (df_signals["ts"] <= end_dt)
        ].copy()
    else:
        df_signals_today = None

    df_trades = _read_trades_sqlite(start_dt, end_dt)

    # --- Metin özet ---
    parts = ["📊 *Gün Sonu Raporu*"]

    if df_signals_today is not None and not df_signals_today.empty:
        up_cnt = int((df_signals_today["trend"] == "Yükseliş").sum())
        dn_cnt = int((df_signals_today["trend"] == "Düşüş").sum())
        parts += [
        ...
        parts += ["• Bugün için sinyal verisi yok veya CSV bulunamadı."]
    ...
    if df_trades is not None and not df_trades.empty:
        ...
        parts += [
            f"• Kapalı İşlem: {len(closed)} | Win/Loss: {win_cnt}/{lose_cnt} (Winrate {winrate:.1f}%)",
            f"• Toplam PnL (USD): {pnl_sum:.2f}",
        ]
    else:
        parts += ["• İşlem kaydı yok."]
    ...
    return summary_text, score_png or equity_png  # en az bir grafik döndürmeye çalış
