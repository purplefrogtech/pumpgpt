...
def _conn():
    # WAL mode hızlı, çoklu işlem güvenli
...
def init_db():
    with _conn() as con:
        ...
        """)
    logger.debug("SQLite tablo hazır (WAL): signals + trades")
    ...
    try:
        with _conn() as con:
            con.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?)
            """, (symbol, price, volume, score, rsi, macd, macd_sig, volume_spike, ts_utc))
        logger.info(f"💾 Sinyal kaydedildi: {symbol} | score={score:.2f}")
    except Exception:
        logger.exception("Signal DB insert hatası:")
...
# --- Trades yardımcıları (değişmedi, sadece exception-safe) ---
def trade_open(symbol, side, entry, size, qty, tp1, tp2, sl, opened_at):
    try:
        ...
        logger.exception("Trade open hatası:")
...
def trade_mark_partial(symbol, qty_delta, last_price, now_ts):
    try:
        ...
        logger.exception("Trade partial hatası:")
...
def trade_close_all(symbol, last_price, now_ts, realized_pnl_usd, realized_pnl_pct):
    try:
        ...
        logger.exception("Trade close hatası:")
